...
read_config_field() {
    # Usage: read_config_field <key> [default]
...
parse_video_ts() {
    ...
    # Also handles B-roll prefixes: B_260312_... or B260312_...
    local name="$1"
    name="${name#B_}"   # strip B_ prefix first
    name="${name#B}"    # then strip bare B prefix
...
is_broll() {
    # Check if a video filename (after pad prefix) is b-roll
    # B-roll files start with B_ or B followed by a digit
    local name="$1"
    if [[ "$name" == B_* ]] || [[ "$name" =~ ^B[0-9] ]]; then
        return 0
    fi
    return 1
}
