...
def adb_connect(device, retries=3, delay=2):
    """Connect to a wireless device with retries. No-op for USB."""
    ...
    for attempt in range(1, retries + 1):
        print(f"  {name}: connecting to {addr} (attempt {attempt}/{retries})…",
              end=" ", flush=True)
        try:
            subprocess.run(["adb", "disconnect", addr],
